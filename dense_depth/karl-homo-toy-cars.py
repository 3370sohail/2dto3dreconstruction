...
for i in range(1,3):
    j = i + 1
    img = cv2.imread("helmet2/h" + str(j) + ".jpg", cv2.IMREAD_GRAYSCALE)
    cv2_imgs.append(img)
...
for i in range(len(cv2_imgs) - 1):
    ...
    H_matrix, matchs = q9.ransac_loop(img1, img2, kp1, kp2, bf_matches, plot=True)
    m_kp1 = []
    m_kp2 = []
...
pcds = []
for i in range(len(inputs)):
    voxel_to_csv( input_voxels[i], './helmet2/depth/h_{}.csv'.format(i))
    pcd = o3d.geometry.PointCloud()
    pcd.points = o3d.utility.Vector3dVector(new_3d_pts[i])
    pcds.append(pcd)
    o3d.io.write_point_cloud("./helmet2/depth/h_{}.pcd".format(i), pcd)
    print("good")
