...
for i in range(len(inputs)):
    ...
    for input_voxel in input_voxels[i]:
        for match in bf_matches:
            ...
            if (match_y > input_voxel_y - window and match_y < input_voxel_y + window) and (
                    match_x > input_voxel_x - window and match_x < input_voxel_x + window):
                temp_input_voxels.append(input_voxel)
                break

    pcd.points = o3d.utility.Vector3dVector(input_voxels[i])
    pcds.append(pcd)
    o3d.io.write_point_cloud("./dump/car_{}.pcd".format(i), pcd)
